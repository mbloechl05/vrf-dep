...
coeff.dia <- get_parameters(fit.simple = fit.dia, i.simple = 79, s.simple = 84, 
                            i.all = 82, s.all = 90)
p2 <- get_plot(coeffs = coeff.dia, title = "Diabetes") # A1SA9X.n

# smoking
coeff.smo <- get_parameters(fit.simple = fit.smo, i.simple = 79, s.simple = 84, 
                            i.all = 79, s.all = 87)
p3 <- get_plot(coeffs = coeff.smo, title = "Current smoking") # A1PA43.n
...
ggsave("output/midus/combined_midus.png", dpi = 600, width = 20, height = 4)
